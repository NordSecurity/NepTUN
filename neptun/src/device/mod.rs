...
pub struct Device {
    ...
    tunnel_rx: Receiver<TunnelWorkerData>,
    tunnel_tx: Sender<TunnelWorkerData>,
}
...
impl Default for TunnelWorkerData {
    fn default() -> Self {
        TunnelWorkerData {
            buffer: [0; PKT_SIZE],
            peer: None,
...
impl Device {
    ...
    fn open_listen_socket(&mut self, mut port: u16) -> Result<(), Error> {
        ...
        thread::spawn(move || tunnel_worker(rx_clone, fw_callback));
            ...
            Box::new(move |d, t| {
                ...
                loop {
                    ...
                    if let Ok(read_bytes) = udp.recv(src_buf) {
                        let mut worker_data = TunnelWorkerData::default();
                        let res = {
                            let mut tun = peer.tunnel.lock();
                            tun.decapsulate(
                                Some(peer_addr),
                                t.src_buf[..read_bytes].as_ref(),
                                &mut worker_data.buffer[..],
                            )
                        };

                        worker_data.peer = Some(peer.clone());
                        worker_data.iface = Some(t.iface.clone());
                        worker_data.udp = Some(udp.clone());
                        worker_data.res = res.into();
                        if let Err(e) = d.tunnel_tx.send(worker_data) {
...
fn tunnel_worker(
    ...
    loop {
        if let Ok(t) = tunnel_rx.recv() {
            let peer = if let Some(p) = t.peer.clone() {
                p
            } else {
                tracing::error!("Empty peer");
                continue;
            ...
            match t.res {
                ...
                DecryptResult::WriteToNetwork(len) => {
                    flush = true;
                    if let Some(udp) = &t.udp {
                        if let Err(err) = udp.send(&t.buffer[..len]) {
                ...
                DecryptResult::WriteToTunnel(len, addr) => {
                    if let Some(callback) = &firewall_process_inbound_callback {
                        if !callback(&peer.public_key.0, &t.buffer[..len]) {
                            continue;
                        }
                    }
                    if peer.is_allowed_ip(addr) {
                        if let Some(iface) = t.iface {
                            _ = iface.as_ref().write(&t.buffer[..len]);
                            tracing::trace!(
                                message = "Writing packet to tunnel",
                                packet_length = len,
                                src_addr = ?addr,
