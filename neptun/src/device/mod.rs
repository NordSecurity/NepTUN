...
mod tests {
    ...
    #[cfg(any(target_os = "macos", target_os = "ios", target_os = "tvos"))]
    fn assert_event_loops_exit(interface_lock: &Lock<Device>) {
        ...
        let exited = loop {
            let closed = interface_lock.read().closed;
            if closed {
                break true;
            }
        ...
        assert!(exited, "event loops did not exit");
    }
    ...
    fn start_event_loop_threads_starts_one_loop_per_thread() {
        ...
        assert_event_loops_exit(&interface_lock);
    }
    ...
    fn udp_handler_updates_peer_endpoint_on_roaming() {
        ...
        let interface_lock = Arc::new(Lock::new(device));
        let (_threads, _sockets_to_close) =
