    ...
    pub async fn run(mut self) {
        ...
        loop {
            tokio::select! {
                ...
                rt = self.plaintext_client.recv_packet(&mut self.plaintext_buf) => {
    ...
    async fn on_recv_cmd(
        ...
        match cmd {
            ...
            TestCmd::SendPlaintext { dst, index } => {
                ...
                self.plaintext_client.send_packet(dst, dst, &payload).await;
            }
        }
    }
    ...
    async fn on_recv_plaintext_packet(
        ...
        } = rt
        {
            if bytes_read == Packet::send_size() {
                ...
                let index = u64::from_le_bytes(self.plaintext_buf[0..8].try_into().unwrap());
                let recv_ts = std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap()
                    .as_micros();
