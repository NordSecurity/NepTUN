...
class Analyzer:
    def __init__(self, csv_path, pcap_path, count, test_type):
        self.count = count
        self.csv_data = CsvData(csv_path)
    ...
    def packet_funnel(self, ax):
        ...
        for p in self.csv_data.packets:
            latencies = p.get_latencies()
            before_wg += 1 if latencies[0] is not None else 0
            after_wg += 1 if latencies[1] is not None else 0
            recv += 1 if latencies[2] is not None else 0
        categories = [
            f"Count ({count})",
